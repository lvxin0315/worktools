...
func Startup(sessionID string) error {
	...
	log.Trace(sessionID, "Startup", "MongoDB : Hosts[%s]", config.String("mongo_hosts"))
	log.Trace(sessionID, "Startup", "MongoDB : Database[%s]", config.String("mongo_database"))
	log.Trace(sessionID, "Startup", "MongoDB : Username[%s]", config.String("mongo_username"))

	hosts := strings.Split(config.String("mongo_hosts"), ",")

	// Create the strong session.
	if err := CreateSession(sessionID, "strong", MasterSession, hosts, config.String("mongo_database"), config.String("mongo_username"), config.String("mongo_password")); err != nil {
		log.CompletedError(err, sessionID, "Startup")
		return err
	}

	// Create the monotonic session.
	if err := CreateSession(sessionID, "monotonic", MonotonicSession, hosts, config.String("mongo_database"), config.String("mongo_username"), config.String("mongo_password")); err != nil {
		log.CompletedError(err, sessionID, "Startup")
		return err
	}
